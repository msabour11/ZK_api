...
@frappe.whitelist(allow_guest=True)
def get_log_dev2():
    ip_address_2 = frappe.db.get_value("Zk Settings", None, 'ip_address_2')
    if ip_address_2:
        frappe.msgprint(ip_address_2)
        url = f"http://10.0.0.117/api/GetAttendance?ipAddress={ip_address_2}&startDate=2024-03-05&endDate=2024-03-06"
        response = requests.get(url)
        if response.status_code == 200:
            data = response.json()
            for record in data:
...
# @frappe.whitelist(allow_guest=True)
# def get_logs(ip_address):
#     url = f"http://10.0.0.117/api/GetAttendance?ipAddress={ip_address}&startDate=2024-03-05&endDate=2024-03-06"
#     response = requests.get(url)
#
#     if response.status_code != 200:
...
#         type1 = type_map.get(in_out_mode, "Unknown")
#
#         if not frappe.db.exists("Device Log", {"enroll_no": record['enrollNumber'], "date": date, "time": record['dateTime']}):
#             doc = frappe.get_doc({
#                 "doctype": "Device Log",
#                 'enroll_no': record['enrollNumber'],
#                 'time': record['dateTime'],
